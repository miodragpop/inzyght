// Block-size chart.
//
// The backend (/api/v1/blocksize/series) returns block size over a height
// range, downsampled to a target point count (averaged per sampled range, since
// size is not cumulative). We render it with uPlot using BLOCK HEIGHT as the
// x-axis (height is the primary axis; the date for a point is shown in the hover
// tooltip). Changing the visible x range — by pan, wheel zoom, or box zoom —
// re-fetches that height window at higher resolution, so zoomed-in views
// approach full per-block detail while the zoomed-out view stays light.
//
...
(function () {
    ...
    const SERIES = { label: 'Block size', color: '#0d6efd', width: 1.5 };

    // Open the chart at the Ycash mining-algorithm change height by default;
    // pre-fork blocks are sparse/atypical, so this gives a more useful view.
    // Reset zoom and panning still reach the full chain.
    const DEFAULT_FROM = 570000;

    const el = {
        chart:    document.getElementById('blocksize-chart'),
    ...
    // Bytes → human string for axis ticks and tooltip.
    function fmtBytes(v) {
        if (v == null) return '–';
        ...
        const opts = {
            ...
            axes: [
                ...
                },
                {
                    ...
                    grid: { stroke: t.grid },
                    size: 70,
                    values: (self, ticks) => ticks.map(v => fmtBytes(v)),
                },
            ],
    ...
    function applyPayload(payload) {
        ...
        const data = [payload.h, payload.size];
    ...
    function loadRange(from, to) {
        ...
        fetch('/api/v1/blocksize/series?' + params.toString())
            .then(r => r.json())
            .then(payload => {
                if (token !== fetchToken) return;   // a newer request superseded this
                if (payload.status !== 'success') {
                    pendingFrom = pendingTo = null;
                    showError(payload.message || 'Failed to load block-size data.');
                    return;
                }
                if (payload.syncing) {
                    pendingFrom = pendingTo = null;
                ...
                if (!payload.h.length) {
                    pendingFrom = pendingTo = null;
                    el.loading.classList.add('d-none');
                    showError('No block-size data available yet.');
                    return;
                }
            ...
            .catch(err => {
                ...
                pendingFrom = pendingTo = null;
                el.loading.classList.add('d-none');
                showError('Network error loading block-size data.');
                console.error(err);
            });
    }
